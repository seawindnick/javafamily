...
public class QuickSort2 {
    ...
    public static void quickSort(int[] arr, int L, int R) {
        ...
        int[] centerIndex = queryCenterIndex(arr, L, R);
        quickSort(arr, L, centerIndex[0] - 1);
        quickSort(arr, centerIndex[1] + 1, R);
    }

    private static int[] queryCenterIndex(int[] arr, int L, int R) {
        int less = L - 1;
        int more = R;//最后一位不参与计算

        int currentIndex = L;
        while (currentIndex < more) {
            if (arr[currentIndex] < arr[R]) {
                ArrayUtil.swap(arr, ++less, currentIndex++);
            } else if (arr[currentIndex] > arr[R]) {
                ArrayUtil.swap(arr, --more, currentIndex);
            } else {
                currentIndex++;
            }
        }
        //将more位置元素与最后一个元素进行置换，将相同的元素最终连在一起
